...
def test_dashboard_reset_owner_only(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # inactive member → denied (303 /home)
    with Session(get_engine()) as db:
        db.query(ProjectMember).filter_by(project_id="pp").delete()
        db.add(
            ProjectMember(
                project_id="pp", user_id="mu", role="owner", tb_user_id="tb", status="disabled"
            )
        )
        db.commit()
    ...
    # owner → allowed (303 back to the project)
    with Session(get_engine()) as db:
        db.query(ProjectMember).filter_by(project_id="pp").delete()
        db.add(
            ProjectMember(
                project_id="pp", user_id="mu", role="owner", tb_user_id="tb", status="active"
            )
        )
        db.commit()
    r = client.post("/projects/pp/dashboard/reset")
