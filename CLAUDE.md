...
- Flows/subdomains: the portal session cookie MUST be parent-domain scoped (`domain=.<DOMAIN>`, set in app/main.py for env!=dev) or the browser won't send it to flows.<domain> and Caddy forward_auth (/flows/auth) returns 401 when you open the editor. `/home` builds 'Open my dashboard' from TB's `POST /api/noauth/oauth2Clients` (needs Host=app.<domain>) so students land via Keycloak SSO, not TB's login form (TB basic-auth login fails 'User account is not active' — students are SSO-only, D3). TB prepends 'Sign in with ' to loginButtonLabel → keep it 'CHERT IoT'. `/docs` (no slash) needs a Caddy `redir /docs /docs/` or it 404s via the portal.
