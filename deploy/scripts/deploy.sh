...
step "ensure lora databases exist (init.sql only runs on a fresh volume)"
$RUN "cd /srv/chertiot && set -a && . ./.env && set +a && [ \"\$LORA_ENABLED\" = true ] || exit 0; docker compose -f docker-compose.yml exec -T postgres psql -U \$POSTGRES_USER -d postgres -tAc \"SELECT 1 FROM pg_database WHERE datname='chirpstack'\" | grep -q 1 || docker compose -f docker-compose.yml exec -T postgres psql -U \$POSTGRES_USER -d postgres -c 'CREATE DATABASE chirpstack'; docker compose -f docker-compose.yml exec -T postgres psql -U \$POSTGRES_USER -d chirpstack -c 'CREATE EXTENSION IF NOT EXISTS pg_trgm; CREATE EXTENSION IF NOT EXISTS hstore'"
